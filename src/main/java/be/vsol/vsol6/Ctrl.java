...
public class Ctrl {
     ...
     * @param primaryStage The main JavaFX-stage, as passed from the JavaFX-launcher
     */
    public Ctrl(Map<String, String> params, Stage primaryStage) {
        ...
        this.gui = new Gui(this, primaryStage);
        this.db = new Db(appConfig);
        this.server = new HttpServer(sig.getAppTitle());
        this.vsol4 = new Vsol4();
        this.orthanc = new Orthanc();

        this.api = switch (appConfig.app.backend) {
            case vsol4 -> new Vsol4API(this);
            case bridge -> null; // TODO new BridgeAPI(this);
            case vsol6 -> null; // TODO new Vsol6API(this);
        };

        if (appConfig.gui.active) gui.showSplashScreen();
    ...
    private void boot(Config appConfig) {
        ...
        if (appConfig.gui.active) startGui(systemSession.getConfig());
    }
    ...
    private void startServer(Config config) {
        server.start(config.server.port, null);
    }
